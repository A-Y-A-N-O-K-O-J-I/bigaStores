...
async function handleSwap(formData) {
  try {
    const response = await axios.post("https://api.kingfix.name.ng/api/swap", formData);
...
function SwapForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const output = await handleSwap(formData); // Uncommented the API call
    ...
    <>
      ...
      <div className="flex justify-center items-center min-h-screen bg-yellow-100 px-4">
        <div className="bg-white shadow-2xl rounded-2xl p-8 max-w-lg w-full space-y-6">
          <h2 className="text-2xl font-semibold text-center text-gray-800">Phone Swap Request</h2>
          <p className="text-center text-sm text-gray-600">
            Fill this form to get your phone swapped for better!
          </p>
