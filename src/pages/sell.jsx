...
async function handleSell(formData) {
  try {
    const response = await axios.post("https://api.kingfix.name.ng/sell", formData);
...
function SellForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const output = await handleSell(formData); // Uncommented the API call
    ...
    <>
      ...
      <div className="flex justify-center items-center min-h-screen bg-yellow-100 px-4">
        <div className="bg-white shadow-2xl rounded-2xl p-8 max-w-lg w-full space-y-6">
          <h2 className="text-2xl font-semibold text-center text-gray-800">Phone Selling Request</h2>
          <p className="text-center text-sm text-gray-600">
            Fill this form to get your phone Sold
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Problem Description */}
            <div>
              <label className="block text-sm font-medium mb-1">Phone Description: </label>
              <textarea
                name="message"
          ...
          <p className="text-center text-xs text-gray-400">We’ll reach out within 24 hours. if we don't please text the whatsapp number</p>
        </div>
      </div>
